...
float4 main(float3 cameraPos : Position, float3 normal : Normal, float2 texCoord : TexCoord) : SV_TARGET
{
    ...
    return float4(saturate(diffuse + ambientColor + specular), 1.0f) * tex.Sample(splr, texCoord);
}